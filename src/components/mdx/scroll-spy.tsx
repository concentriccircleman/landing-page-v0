...
const getDefaultScrollBehavior = (): ScrollBehavior => {
...
const useStore = <T,>(selector: (state: StoreState) => T, ogStore?: Store | null): T => {
...
const useScrollSpyContext = (consumerName: string) => {
...
const ScrollSpy = (props: ScrollSpyProps) => {
  ...
  }, [value, defaultValue, onScrollToSection, store]);
...
const ScrollSpyNav = (props: ScrollSpyNavProps) => {
...
const ScrollSpyLink = (props: ScrollSpyLinkProps) => {
  ...
  const isActive = activeValue === linkValue;
  ...
  return (
    <LinkPrimitive
      ...
      className={cn(
        "block text-sm font-semibold text-muted hover:text-foreground duration-200 data-[state=active]:text-foreground",
        className,
      )}
      onClick={onLinkClick}
    />
...
const ScrollSpyViewport = (props: ScrollSpyViewportProps) => {
...
export { ScrollSpy, ScrollSpyLink, ScrollSpyNav, ScrollSpySection, ScrollSpyViewport, type ScrollSpyProps };
