            ...
            <div className={styles.detailActionCallout}>
              ...
              <div className={styles.detailActionButtons}>
                ...
                <Button variant="ghost" size="sm" onClick={() => {
                  const mailto = `mailto:${alert.people.map(p => p.name.toLowerCase().replace(/\s+/g, '.') + '@sentra.ai').join(',')}?subject=Re: ${encodeURIComponent(alert.title)}&body=${encodeURIComponent(alert.suggestedAction)}`;
                  window.open(mailto);
                }}>
                  Send Email
                </Button>
                <Button variant="ghost" size="sm" onClick={() => {}}>
                  <CheckCircle width={13} height={13} /> Create Task
                </Button>
              </div>
...
export function RiskRadarPage({ onOpenChat }: { onOpenChat?: () => void } = {}) {
  ...
  const [severityFilter] = useState<Set<string>>(new Set());
    ...
    <div className={styles.page}>
        ...
        actions={
          <>
            ...
            <Button variant="primary" size="sm" onClick={() => setCreateOpen(true)}>
              <PlusIcon width={14} height={14} /> New Radar
            </Button>
            {onOpenChat && (
      ...
      {settingsOpen && (
        <div className={styles.modalBackdrop} onClick={() => setSettingsOpen(false)}>
          <div className={styles.settingsModal} onClick={(e) => e.stopPropagation()}>
            <h3 className={styles.modalTitle}>Radar Settings</h3>
            <Separator />
            <label className={styles.settingRow}><span>Real-time alerts</span><input type="checkbox" defaultChecked /></label>
            <label className={styles.settingRow}><span>Email digest (daily)</span><input type="checkbox" defaultChecked /></label>
            <label className={styles.settingRow}><span>Slack notifications</span><input type="checkbox" /></label>
            <label className={styles.settingRow}><span>Auto-snooze resolved risks</span><input type="checkbox" defaultChecked /></label>
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "var(--space-3)" }}>
              <Button variant="primary" size="sm" onClick={() => setSettingsOpen(false)}>Save</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {createOpen && (
        <div className={styles.modalBackdrop} onClick={() => setCreateOpen(false)}>
          <div className={styles.settingsModal} onClick={(e) => e.stopPropagation()}>
            <h3 className={styles.modalTitle}>Create New Radar</h3>
            <Separator />
            <div className={styles.settingRow}>
              <span>Topic</span>
              <Input placeholder="e.g. Q2 Budget Risks" size="sm" />
            </div>
            <div className={styles.settingRow}>
              <span>Severity threshold</span>
              <Select value="medium" size="sm">
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "var(--space-2)", marginTop: "var(--space-3)" }}>
              ...
              <Button variant="primary" size="sm" onClick={() => setCreateOpen(false)}>Create Radar</Button>
            </div>
          </div>
        </div>
      )}
