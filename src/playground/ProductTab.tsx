...
import { ExperimentsProvider, useExperiments, EXPERIMENT_FLAGS } from "./ExperimentsContext";
import { SunFilled, MoonFilled } from "../icons/filled";
import { AiSparkle } from "../icons/outline";
import styles from "./ProductTab.module.css";
import sidebarStyles from "../components/SidebarNav/SidebarNav.module.css";
...
function ActivePage({ page, sageMeetingCompleted, onOpenChat, onNavigate, onNavigateToSage }: { page: string; sageMeetingCompleted?: boolean; onOpenChat?: () => void; onNavigate?: (page: string) => void; onNavigateToSage?: () => void }) {
  switch (page) {
    case "home": return <HomePage onOpenChat={onOpenChat} onNavigate={onNavigate} />;
    case "weekly-reports": return <WeeklyReportsPage onOpenChat={onOpenChat} />;
    case "meeting-notes": return <MeetingNotesPage sageMeetingCompleted={sageMeetingCompleted} onOpenChat={onOpenChat} onNavigateToSage={onNavigateToSage} />;
    case "todos": return <TodosPage onOpenChat={onOpenChat} />;
    case "memory": return <MemoryPage onOpenChat={onOpenChat} onNavigate={onNavigate} />;
...
function ExperimentsContent() {
  ...
  return (
    <>
      {EXPERIMENT_FLAGS.map((exp) => (
        <div key={exp.key} className={sidebarStyles.experimentRow}>
          <div className={sidebarStyles.experimentInfo}>
            <span className={sidebarStyles.experimentLabel}>{exp.label}</span>
            <span className={sidebarStyles.experimentDesc}>{exp.description}</span>
          </div>
          <Switch
            checked={flags[exp.key] ?? exp.defaultValue}
            onCheckedChange={(v) => setFlag(exp.key, v)}
          />
        </div>
      ))}
    </>
...
function ProductShell({ sageMeetingCompleted, initialPage, onNavigateToSage }: { sageMeetingCompleted?: boolean; initialPage?: string; onNavigateToSage?: () => void }) {
  ...
  const sections: NavSection[] = useMemo(() => [
    {
      ...
      label: "Workspace",
      items: [
        { id: "memory", label: "Memory", icon: "evaluations" as const },
        { id: "weekly-reports", label: "Weekly Reports", icon: "reports" as const },
        { id: "meeting-notes", label: "Meeting Notes", icon: "reviews" as const },
        { id: "deep-research", label: "Deep Research", icon: "workflows" as const },
        { id: "risk-radar", label: "Risk Radar", icon: "alerts" as const },
        { id: "reports", label: "Reports", icon: "reports" as const },
      ],
    },
    {
      id: "personal",
      label: "Personal",
      items: [
        { id: "todos", label: "Commitments", icon: "assignments" as const },
        { id: "apps", label: "Apps", icon: "calibrations" as const },
      ],
    },
    {
      id: "experiments",
      ...
      customContent: <ExperimentsContent />,
    },
  ], []);
      ...
      >
        <div className={styles.pageRow} ref={rowRef}>
          <div className={styles.pageMain}>
            <ActivePage page={activeNav} sageMeetingCompleted={sageMeetingCompleted} onOpenChat={toggleResearch} onNavigate={handleNavClick} onNavigateToSage={onNavigateToSage} />
          </div>
          {showPanel && (
            <>
              <div
...
export function ProductTab({ sageMeetingCompleted, initialPage, onNavigateToSage }: { sageMeetingCompleted?: boolean; initialPage?: string; onNavigateToSage?: () => void } = {}) {
  return (
    <ThemeProvider>
      <DemoProvider>
        <ExperimentsProvider>
          <ResearchProvider>
            <ProductShell sageMeetingCompleted={sageMeetingCompleted} initialPage={initialPage} onNavigateToSage={onNavigateToSage} />
          </ResearchProvider>
        </ExperimentsProvider>
      </DemoProvider>
    </ThemeProvider>
